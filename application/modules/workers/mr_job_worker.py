...
def mrjobworker(request_id):
	try:
	    ...
	    maprjob_data = db_session.query(TblMrJobInfo.var_resourcemanager_ip,TblMrJobInfo.uid_customer_id,TblMrJobInfo.uid_cluster_id,TblMrJobInfo.var_file_name,TblMrJobInfo.var_job_description).filter(TblMrJobInfo.uid_request_id == request_id).all()
	    ...
	    job_description =maprjob_data[0][4]
	    mapred = MapRedResourceManager(address=IPAddr, port=8088)
	    application_id=mapred.submitJob(jar_path="/opt/mnt/azurefileshare/sampledir/"+file_name,filename=job_description,input="/file",output="/test/job"+time_stamp)
